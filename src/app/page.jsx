	...
	return (
		<section className=" flex-1  text-justify">
			...
			<p className=" font-alltext text-sm lg:text-lg text-slate-900 dark:text-slate-300"> I am a student at the Malawi univerisity of business and applied science. I am pursuing a  Bachelors degree in Information Technology.
				I am a minimalist and a linux fan who recently survived a <a className="hover:underline text-emerald-900  dark:text-[#9DA9A0]" target="_blank" href="https://www.reddit.com/r/unixporn/comments/3iy3wd/stupid_question_what_is_ricing/">ricing</a> addiction. I write code as an excuse to use vim.<a href="https://archlinux.org/" className=" hover:underline text-emerald-900 dark:text-[#9DA9A0]">Arch-linux(btw) </a>,<a href="https://neovim.io/" className=" hover:underline text-emerald-900  dark:text-[#9DA9A0]">Neovim</a>,<a href="https://github.com/kovidgoyal/kitty" className="dark:text-[#9DA9A0]  text-emerald-900  hover:underline">Kitty</a>, <a href="https://github.com/zellij-org/zellij" className="hover:underline text-emerald-900 dark:text-[#9DA9A0]">zellij</a> and <a href="https://github.com/zellij-org/zellij" className="hover:underline text-emerald-900 dark:text-[#9DA9A0]">i3 supremancy</a></p>
		</section>
	)
...
export function Card({ title, date, description, image, link, time, timetoread }) {

	return (
		<>
			<section className="border  rounded-md md:flex hidden">
				<section className="flex flex-col flex-1  p-4">
					<Link href={"/" + link} className="lg:text-xl text-md dark:text-[#A7C080] text-emerald-900 hover:underline">{title}</Link>
					<p className="dark:text-slate-400 text-[#232A2E] text-xs lg:text-sm">{date}</p>
					<p className="text-sm lg:text-md">{description}</p>
					<p className="dark:text-slate-500 text-[#3A515D] italic text-xs  ">{timetoread + " min read"}</p>
					...
					<section className="flex backdrop-blur-lg rounded-md">
						<div className="flex p-2 gap-4">
							...
							<div className="flex gap-1 font-Quicksand justify-center items-start flex-col ">
								<div className="text-xs flex gap-2"> {date}<p>{"⏱️" + timetoread + "min read"}</p></div>
								<h3 className="text-md dark:text-[#A7C080] font-bold">{title}</h3>
								<p className="text-sm text-[#3A515D] dark:text-slate-400 italic ">{description}</p>
							</div>
...
export function BlogPosts() {
	...
	for (let i = 0; i < posts.length; i++) {
		cards.push(<Card key={i} title={posts[i].title} date={posts[i].date} description={posts[i].description} timetoread={posts[i].timetoread} image={posts[i].image} link={posts[i].link} />)
	}
	return (
